...
int main()
{
  ...
  // Ensure we can open and write to the backing store
  FILE *fp;
  fp = fopen("BACKING_STORE_RW.bin","wb");
  ...
  FILE *addresses = fopen("addresses2.txt","r");
  char buf[256];
  int current_time = 0;
  while(fgets(buf,sizeof(buf),addresses) != NULL)
  {
    ...
    char *num_section = strtok(buf_cpy, " ");
    char *rw_section = strtok(NULL, " ");

    // Remove any newline from rw section
    strtok(rw_section,"\n");

    // Boolean flag to see if this page is marked as written and will need to be flushed to the backing store before being overwritten
    int is_w = (strcmp(rw_section,"W") == 0);

    uint32_t v_addr = strtoul(num_section,NULL,10);
    ...
    if (!page_table_hit && !tlb_hit)
    {
      ...
      // If we see that the memory has been written to, we need to flush it to the backing store at the relevant page
      // First, we find the page number associated with this frame

      if (is_w)
      {
        int page_flush_num = 0;
        for(int i=0; i<num_pages; i++)
        {
          if(page_table_values[i] == frame_num)
          {
            page_flush_num = page_table_keys[i];
            break;
          }
        }

        // Normally we would write the contents now of physical_memory[frame_num] to page_flush_num of the backing store
        // However as far as I can see, this isn't actually modified in this assignment. So we just calculate statistics
        page_flush_count++; 
